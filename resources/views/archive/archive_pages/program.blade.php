...
'class'=>'form-horizontal']) !!}
    ...
    <div id="demo" class="collapse <?php echo !empty($_GET['column']) ? 'in' : ''; ?>">
        <div class="form-group">
            <div class="col-md-12">
                <br/>
                <?php
                $extra_column = get_program_extra_column();
    ...
    <div class="form-group">
        ...
        <div class="col-md-2">
            ...
            <input type="hidden" name="archive_type" value="4"/>
            <?php
            $item_type = get_program_item_key();
            foreach ($item_type as $key_id) {

                if ((isset($_GET['item_type']) && $_GET['item_type'] == $key_id) || isset($_GET[get_program_key_name($key_id)])) {

                    if (empty($_GET[get_program_key_name($key_id)])) {
                        $_REQUEST[get_program_key_name($key_id)] = [];
                    }
                    if (isset($_REQUEST['search']) && ($_GET['item_type'] == $key_id && (!empty($_GET['search_string'])))) {
                        $_REQUEST[get_program_key_name($key_id)][] = $_GET['search_string'];
                    }
                    if (!empty($_REQUEST[get_program_key_name($key_id)])) {
                        foreach (array_unique($_REQUEST[get_program_key_name($key_id)]) as $key => $value) {
                            echo '<input type="hidden" name="' . get_program_key_name($key_id) . '[]" value="' . $value . '"/>';
                        }
                    }
                }
            }
            ...
            <select class="form-control" name="item_type">
                ...
                <?php  $search_item = program_search_item()  ?>
        ...
        <div class="col-md-4">
            ...
            <a href="{{ url('archive_book?archive_type=4') }}">
...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        foreach($item_type as $key_id) {
        if(!empty($_GET[get_program_key_name($key_id)]) || (!empty($_GET['item_type']) && $_GET['item_type'] == $key_id && (!empty($_GET['search_string']))) ) {
        ?>
        <span class="badge">
                                                {{ get_program_key_text($key_id)  }}
            <?php
            foreach(array_unique($_REQUEST[get_program_key_name($key_id)]) as $index  =>  $value) {
            echo $value;
            ?>
                                                <a href="{{ get_request_url(get_program_key_name($key_id),$index)  }}">
                                                    <i style="color:darkred" class="fa fa-close"></i>
                                                </a>
                                                <?php
            }
...
<div class="col-sm-12">
    <table id="dt_archive__" class="table table-striped table-bordered table-hover"
        ...
        ?>
        <tr>
            <td>{{ !empty($item_info->name) ? $item_info->name : '' }}</td>
            <td>{{ !empty($item_info->category)?get_name_by_id('archive_category',['id'=>$item_info->category])->name:'' }}</td>
            <td>{{ !empty($item_info->gobeshona)  ? get_shilpi_name_by_ids($item_info->gobeshona) : ''  }}</td>
            <td>{{ !empty($item_info->gronthona)  ? get_shilpi_name_by_ids($item_info->gronthona) : ''  }}</td>
            <td>{{ !empty($item_info->uposthapona)  ? get_shilpi_name_by_ids($item_info->uposthapona) : ''  }}</td>
