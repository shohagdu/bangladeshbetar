{!! Form::open(['url' => '/save_employee_award_info', 'method' => 'post', 'id' => 'employee_award_form',
            ...
            <table class="table table-bordered width100per">
                ...
                @if(!empty($award_info))
                    ...
                    @foreach($award_info as $award)
                        <tr>
                            <td><input type="text" name="title[]" placeholder="Award Title" value="{{ (!empty
                            ($award['title']))?$award['title']:'' }}" class="form-control" id="gchildName_{{ $kl
                            }}"></td>
                            <td><textarea name="description[]"  rows="1" placeholder="Award Description"
                                          class="form-control"
                                          id="description_{{ $kl }}">{{ (!empty($award['description']))
