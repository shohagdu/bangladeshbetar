{!! Form::open(['url' => '/save_employee_expertise_info', 'method' => 'post', 'id' => 'employee_expertise_info_form',
            ...
            <table class="table table-bordered width100per">
                ...
                @if(!empty($expertise_info))
                    ...
                    @foreach($expertise_info as $expertise)
                        <tr>
                            <td><input type="text" name="title[]" placeholder="Title" value="{{ (!empty
                            ($expertise['title']))?$expertise['title']:'' }}" class="form-control" id="gchildName_{{ $kl
                            }}"></td>
                            <td><textarea name="description[]"  rows="1" placeholder="Description" class="form-control"
                                       id="description_{{ $kl }}">{{ (!empty($expertise['description']))
