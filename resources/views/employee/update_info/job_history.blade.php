{!! Form::open(['url' => '/save_employee_job_hisotry', 'method' => 'post', 'id' => 'employee_job_hisotry_form','class'=>'form-horizontal']) !!}
<div class="modal-body">
    <div class="col-sm-12">
        <table class="table table-bordered width100per">
            ...
            <tr>
                <td>
                    <input type="text" value="{{ (!empty($job['organigation']))?$job['organigation']:'' }}" name="organigation[]" placeholder="Organigation" class="form-control" id="organigation_{{ $i }}">
                </td>
                <td><input type="text" value="{{ (!empty($job['post']))?$job['post']:'' }}"  name="post[]" placeholder="Post" class="form-control" id="post_{{ $i }}"></td>
                <td><input type="text" value="{{ (!empty($job['office_address']))?$job['office_address']:'' }}"  name="office_address[]" placeholder="Office Address" class="form-control" id="office_address_{{ $i }}"></td>
                <td><input type="text" value="{{ (!empty($job['department']))?$job['department']:'' }}"  name="department[]" placeholder="Department" class="form-control" id="department_{{ $i }}"></td>
                <td><input type="text"  value="{{ (!empty($job['job_from_date']))?$job['job_from_date']:'' }}"  name="job_from_date[]" placeholder="From Date" class="form-control datepickerinfo" id="job_from_date_{{ $i }}"></td>
                <td><input type="text" value="{{ (!empty($job['job_to_date']))?$job['job_to_date']:'' }}"  name="job_to_date[]" placeholder="To Date" class="form-control datepickerinfo" id="job_to_date_{{ $i }}"></td>
                <td><input type="text" value="{{ (!empty($job['job_payscale']))?$job['job_payscale']:'' }}"  name="job_payscale[]" placeholder="Pay Scale" class="form-control" id="job_payscale_{{ $i }}"></td>
                <td><a href="javascript:void(0);"  id="deleteRow_{{ $i }}"  class="deleteRow btn btn-warning btn-flat btn-sm"><i class="glyphicon glyphicon-remove"></i> Drop</a></td>
