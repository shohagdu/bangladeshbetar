...
<table id="print_table" rules="all" border="1px" class="table table-bordered"  style="width:100%;float: left;">
    ...
    <tr>
        <td>  {{ $i++  }}</td>
        <td><input type="checkbox" name="employee_id[{{$key  }}]" id="employee_id" value=" {{ $payrole_info->employee_id }}"></td>
        ...
        <td>{{ number_format($payable-$deduction,2,'.','') }}</td>
        <td>
            <button type="button" class="btn btn-xs btn-info" data-toggle="modal" data-target="#paySlipViewModal"  title="View Payslip" onclick="payslipView('{{ $payrole_info->payslip_id }}')"><i class="glyphicon glyphicon-share-alt"></i> View</button>
        </td>
