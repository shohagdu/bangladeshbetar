...
'class'=>'form-horizontal']); ?>
    ...
    <div id="demo" class="collapse <?php echo !empty($_GET['column']) ? 'in' : ''; ?>">
        <div class="form-group">
            <div class="col-md-12">
                <br/>
                <?php
                $extra_column = get_natok_extra_column();
    ...
    <div class="form-group">
        ...
        <div class="col-md-2">
            ...
            <input type="hidden" name="archive_type" value="3"/>
            <?php
            $item_type =get_natok_item_key();
            foreach ($item_type as $key_id) {

                if ((isset($_GET['item_type']) && $_GET['item_type'] == $key_id) || isset($_GET[get_natok_key_name($key_id)])) {

                    if (empty($_GET[get_natok_key_name($key_id)])) {
                        $_REQUEST[get_natok_key_name($key_id)] = [];
                    }
                    if (isset($_REQUEST['search']) && ($_GET['item_type'] == $key_id && (!empty($_GET['search_string'])))) {
                        $_REQUEST[get_natok_key_name($key_id)][] = $_GET['search_string'];
                    }
                    if (!empty($_REQUEST[get_natok_key_name($key_id)])) {
                        foreach (array_unique($_REQUEST[get_natok_key_name($key_id)]) as $key => $value) {
                            echo '<input type="hidden" name="' . get_natok_key_name($key_id) . '[]" value="' . $value . '"/>';
                        }
                    }
                }
            }
            ...
            <select class="form-control" name="item_type">
                ...
                <?php  $search_item = natok_search_item()  ?>
        ...
        <div class="col-md-4">
            ...
            <a href="<?php echo e(url('archive_book?archive_type=3')); ?>">
...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        foreach($item_type as $key_id) {
        if(!empty($_GET[get_natok_key_name($key_id)]) || (!empty($_GET['item_type']) && $_GET['item_type'] == $key_id && (!empty($_GET['search_string']))) ) {
        ?>
        <span class="badge">
                                                <?php echo e(get_natok_key_text($key_id)); ?>

            <?php
            foreach(array_unique($_REQUEST[get_natok_key_name($key_id)]) as $index  =>  $value) {
            echo $value;
            ?>
                                                <a href="<?php echo e(get_request_url(get_natok_key_name($key_id),$index)); ?>">
                                                    <i style="color:darkred" class="fa fa-close"></i>
                                                </a>
                                                <?php
            }
...
<div class="col-sm-12">
    <table id="dt_archive__" class="table table-striped table-bordered table-hover"
            ...
            <tr>
                ...
                if (!empty($_GET['column'])) {
                    foreach ($_GET['column'] as $value) {
                        echo "<th>" . get_natok_column_title($value) . "</th>";
                    }
                }
                ?>
        ...
        $item_info = json_decode($row->natok_info);
        ?>
        <tr>
            <td>
            ...
            <td>
                <?php echo e(!empty($item_info->actors)  ? get_shilpi_name_by_ids($item_info->actors) : ''); ?>

            </td>
            <td>
                <?php echo e(!empty($item_info->nattokar)  ? get_shilpi_name_by_ids($item_info->nattokar) : ''); ?>

            </td>
            <td>
                <?php echo e(!empty($item_info->rupokar)  ? get_shilpi_name_by_ids($item_info->rupokar) : ''); ?>

            </td>
            <td>
                <?php echo e(!empty($item_info->natok_producer)  ? get_shilpi_name_by_ids($item_info->natok_producer) : ''); ?>
            ...
            if (!empty($_GET['column'])) {
                foreach ($_GET['column'] as $value) {
                    ...
                    elseif ($value == 'category') {
                        echo !empty($item_info->category) ? get_name_by_ids('archive_category', [$item_info->category]) : '';
                    }
