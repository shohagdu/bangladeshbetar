...
'class'=>'form-horizontal']); ?>
    ...
    <div id="demo" class="collapse <?php echo !empty($_GET['column']) ? 'in' : ''; ?>">
        <div class="form-group">
            <div class="col-md-12">
                <br/>
                <?php
                $extra_column = get_sakhhatkar_extra_column();
    ...
    <div class="form-group">
        ...
        <div class="col-md-2">
            <input type="hidden" name="archive_type" value="6"/>
            <?php  $search_item = get_sakhhatkar_item_key();
            foreach ($search_item as $key_id) {

                if ((isset($_GET['item_type']) && $_GET['item_type'] == $key_id) || isset($_GET[get_sakhhatkar_key_name($key_id)])) {

                    if (empty($_GET[get_sakhhatkar_key_name($key_id)])) {
                        $_REQUEST[get_sakhhatkar_key_name($key_id)] = [];
                    }
                    if (isset($_REQUEST['search']) && ($_GET['item_type'] == $key_id && (!empty($_GET['search_string'])))) {
                        $_REQUEST[get_sakhhatkar_key_name($key_id)][] = $_GET['search_string'];
                    }
                    if (!empty($_REQUEST[get_sakhhatkar_key_name($key_id)])) {
                        foreach (array_unique($_REQUEST[get_sakhhatkar_key_name($key_id)]) as $key => $value) {
                            echo '<input type="hidden" name="' . get_sakhhatkar_key_name($key_id) . '[]" value="' . $value . '"/>';
                        }
                    }
                }
            }
            ...
            <select class="form-control" name="item_type">
                ...
                <?php
                $sakhhatkar_item_info = sakhhatkar_search_item();

                ?>
                <?php $__currentLoopData = $sakhhatkar_item_info; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $name): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="col-md-4">
            ...
            <a href="<?php echo e(url('archive_book?archive_type=6')); ?>">
...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        foreach($search_item as $key_id) {
        if(!empty($_GET[get_sakhhatkar_key_name($key_id)]) || (!empty($_GET['item_type']) && $_GET['item_type'] == $key_id && (!empty($_GET['search_string']))) ) {
        ?>
        <span class="badge">
                                                <?php echo e(get_sakhhatkar_key_text($key_id)); ?>

            <?php
            foreach(array_unique($_REQUEST[get_sakhhatkar_key_name($key_id)]) as $index  =>  $value) {
            echo $value;
            ?>
                                                <a href="<?php echo e(get_request_url(get_sakhhatkar_key_name($key_id),$index)); ?>">
                                                    <i style="color:darkred" class="fa fa-close"></i>
                                                </a>
                                                <?php
            }
...
<br/>
<div class="col-sm-12">
    <table id="dt_archive__"  class="table table-striped table-bordered table-hover"
           width="100%">
        <thead>
        <tr>
            <th>প্রথম লাইন</th>
        ...
        $item_info = json_decode($row->sakhhatkar_info);
                ...
                ?>
            <tr>
                <td><?php echo e(!empty($item_info->first_line) ? $item_info->first_line : ''); ?></td>
                <td><?php echo e(!empty($item_info->program_name) ? $item_info->program_name : ''); ?></td>
                <td><?php echo e(!empty($item_info->sakhhatkar_department) ? get_sakhhatkar_department_name($item_info->sakhhatkar_department) :''); ?></td>
                <td><?php echo e(!empty($item_info->category)?get_name_by_id('archive_category',['id'=>$item_info->category])->name:''); ?></td>
                <td><?php echo e(!empty($item_info->sakhhat_data) ? $item_info->sakhhat_data : ''); ?></td>
