...
'class'=>'form-horizontal']); ?>
    ...
    <div id="demo" class="collapse <?php echo !empty($_GET['column']) ? 'in' : ''; ?>">
        <div class="form-group">
            <div class="col-md-12">
                <br/>
                <?php
                $extra_column = get_vhason_extra_column();
    ...
    <div class="form-group">
        ...
        <div class="col-md-2">
            ...
            <input type="hidden" name="archive_type" value="5"/>
            <?php
            $item_type = get_vhason_item_key();
            foreach ($item_type as $key_id) {

                if ((isset($_GET['item_type']) && $_GET['item_type'] == $key_id) || isset($_GET[get_vhason_key_name($key_id)])) {

                    if (empty($_GET[get_vhason_key_name($key_id)])) {
                        $_REQUEST[get_vhason_key_name($key_id)] = [];
                    }
                    if (isset($_REQUEST['search']) && ($_GET['item_type'] == $key_id && (!empty($_GET['search_string'])))) {
                        $_REQUEST[get_vhason_key_name($key_id)][] = $_GET['search_string'];
                    }
                    if (!empty($_REQUEST[get_vhason_key_name($key_id)])) {
                        foreach (array_unique($_REQUEST[get_vhason_key_name($key_id)]) as $key => $value) {
                            echo '<input type="hidden" name="' . get_vhason_key_name($key_id) . '[]" value="' . $value . '"/>';
                        }
                    }
                }
            }
            ...
            <select class="form-control" name="item_type">
                ...
                <?php  $search_item = vhason_search_item()  ?>
        ...
        <div class="col-md-4">
            ...
            <a href="<?php echo e(url('archive_book?archive_type=5')); ?>">
...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        foreach($item_type as $key_id) {
        if(!empty($_GET[get_vhason_key_name($key_id)]) || (!empty($_GET['item_type']) && $_GET['item_type'] == $key_id && (!empty($_GET['search_string']))) ) {
        ?>
        <span class="badge">
                                                <?php echo e(get_vhason_key_text($key_id)); ?>

            <?php
            foreach(array_unique($_REQUEST[get_vhason_key_name($key_id)]) as $index  =>  $value) {
            echo $value;
            ?>
                                                <a href="<?php echo e(get_request_url(get_vhason_key_name($key_id),$index)); ?>">
                                                    <i style="color:darkred" class="fa fa-close"></i>
                                                </a>
                                                <?php
            }
...
<div class="col-sm-12">
    <table id="dt_archive__" class="table table-striped table-bordered table-hover"
        ...
        <tr>
            ...
            if (!empty($_GET['column'])) {
                foreach ($_GET['column'] as $value) {
                    echo "<th>" . get_vhason_column_title($value) . "</th>";
                }
            }
            ?>
        ...
        foreach($archive_list as $row) {
            $item_info = json_decode($row->vhason_info);
        ?>
        <tr>
        <td>
            <?php echo e($item_info->first_line); ?>

        </td>
        <td><?php echo e(!empty($item_info->category)?get_name_by_id('archive_category',['id'=>$item_info->category])->name:''); ?></td>
        <td><?php echo e($item_info->vhason_kari); ?></td>
        <td><?php echo e($item_info->designation); ?></td>
        <td><?php echo e($item_info->prodaner_tarikh); ?></td>
        ...
        if (!empty($_GET['column'])) {
            foreach ($_GET['column'] as $value) {
                echo "<td>";
                if ($value == 'subject') {
                    echo !empty($item_info->subject) ? $item_info->subject : '';
                }
